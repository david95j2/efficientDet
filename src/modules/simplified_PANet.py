...
class Simplified_PANet(nn.modules):
    def __init__(self, num_channels, epsilon=1e-4):
        ...
        self.p5_w2 = nn.Parameter(torch.ones(2))
    ...
    def forward(self, inputs):
        ...
        # ---------------------------------------------------------------------------------------- Weights : P4_1, P3_2 --> P4_2
        p4_w2 = self.p4_w2_relu(self.p4_w2)
        weight = p4_w2 / (torch.sum(p4_w2, dim=0) + self.epsilon)
        p4_out = self.conv4_down(weight[1] * p4_up + weight[2] * self.p4_downsample(p3_out)) # Connections
        
        
        # ---------------------------------------------------------------------------------------- Weights : P5_1, P4_2 --> P5_2
        p5_w2 = self.p5_w2_relu(self.p5_w2)
        weight = p5_w2 / (torch.sum(p5_w2, dim=0) + self.epsilon)
        p5_out = self.conv5_down(weight[1] * p5_up + weight[2] * self.p5_downsample(p4_out)) # Connections
        
        
        # ---------------------------------------------------------------------------------------- Weights : P6_1, P5_2 --> P6_2
        p6_w2 = self.p6_w2_relu(self.p6_w2)
        weight = p6_w2 / (torch.sum(p6_w2, dim=0) + self.epsilon)
        p6_out = self.conv6_down(weight[1] * p6_up + weight[2] * self.p6_downsample(p5_out)) # Connections
        
        
        # ---------------------------------------------------------------------------------------- Weights : P7_0, P6_2 --> P7_2
